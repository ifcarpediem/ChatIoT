...
from backend.agents.actions.action import Action
...
class Role():
    ...
    async def _act(self, msg: Message) -> Message:
        ...
        logger.info(f"{self._setting} rsp_msg {rsp_msg.to_dict()}")
    ...
    async def _observe(self) -> int:
        ...
        logger.info(f'{self._setting} env_msgs: {env_msgs}')
        ...
        logger.info(f'{self._setting} received: {received}')
        logger.info(f'{self._setting} history: {self._rc.history}')

        news = self._rc.memory.find_news(received)
        logger.info(f'{self._setting} news: {news}')
        ...
        if news_text:
            logger.debug(f'{self._setting} received: {news_text}')
            return news[-1]
        return None
    ...
    async def run(self, message=None):
        """观察，并基于观察的结果思考、行动"""
        logger.info(f"{self._setting} running.\nmessage: {message}\nnow history: {self._rc.history}")
        new_message = await self._observe()
        if new_message:
            logger.info(f"{self._setting} new_message: {new_message.to_dict()}")
        if message:
        ...
        logger.info(f"{self._setting} rsp_message: {rsp_message.to_dict()}")
        # 将回复发布到环境，等待下一个订阅者处理
